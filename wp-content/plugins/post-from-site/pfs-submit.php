...
function pfs_submit($post,$files){
    ...
    $title          = $pfs_data['name'];
    $postcontent    = $pfs_data['model'];
    $nameCommunity  = $pfs_data['name'];
    $colorCommunity = $pfs_data['color'];
    $cityCommunity  = $pfs_data['city'];
    $modelCommunity = $pfs_data['model'];
    ...
    if ( !current_user_can('publish_posts') && $pfs_options['allow_anon'] && $pfs_options['enable_captcha'] && !$resp->is_valid ) {
        ...
        if ( $has_content_things ) {
            ...
            add_post_meta($post_id, "name", $nameCommunity, false);
            add_post_meta($post_id, "model", $modelCommunity, false);
            add_post_meta($post_id, "city", $cityCommunity, false);
            add_post_meta($post_id, "color", $colorCommunity, false);
            add_post_meta($post_id, "email", $email, false);
            add_post_meta($post_id, "bikeimage", $upload[1], false);
